...
startBtn.addEventListener("click", function () {
  ...
  const { transformedSecs, transformedMins, transformedHrs } = tranformInputs(
    secs,
    mins,
    hrs
  );
  console.log("transformedMins ", transformedMins);
  console.log("transformedSecs ", transformedSecs);
  console.log("transformedHrs ", transformedHrs);
...
function timer(countDownTime) {
  ...
  counterID = setInterval(() => {
    // a. process transformed  input after every one second
    countDownTime--;
...
function updateUIEverySec(countDownTime) {
  console.log("inside updateUiEverySec ", countDownTime);
  //1st way
  // 1. countDOWNTIME:-convert into hrs:mins:secs and put it to the UI
...
// Always keep your UI and data into sync .This is known as 2 way binding
