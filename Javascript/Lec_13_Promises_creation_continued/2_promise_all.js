Promise.myPromiseALL = function (arrayOFPromises) {
  return new Promise(function (resolve, reject) {
    ...
    //  what does a promise represent??  Ans:- future value of the task that has already started
    // Since the promise represent future value of the task that represent so we are assuming that every promises in the arrayOFPromises are pending so use async await and iterate over the array and if the value is not resolved then await for that
    //and if it is resolved then await for that
